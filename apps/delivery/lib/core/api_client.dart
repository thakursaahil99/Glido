...
/// Same origin the web/customer app talks to — override with
/// --dart-define=API_BASE_URL=... when running against a device that can't
/// reach localhost directly (e.g. a real phone needs the host's LAN IP).
...
/// Thin Dio wrapper — persists JWTs, retries once on 401 after a token refresh.
class ApiClient {
  ApiClient._internal() {
  ...
  Future<void> loadTokens() async {
    ...
    _accessToken = prefs.getString('glido_delivery_access_token');
    _refreshToken = prefs.getString('glido_delivery_refresh_token');
  }
  ...
  Future<void> setTokens(String access, String refresh) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('glido_delivery_access_token', access);
    await prefs.setString('glido_delivery_refresh_token', refresh);
  }
  ...
  Future<void> clearTokens() async {
    ...
    _refreshToken = null;
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove('glido_delivery_access_token');
    await prefs.remove('glido_delivery_refresh_token');
  }
