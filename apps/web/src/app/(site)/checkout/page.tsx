...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    loadAddresses();
    api
      .get<Restaurant>(`/restaurants/${cart.restaurantId}`)
      .then(setRestaurant)
      .catch(() => undefined);
    api
  ...
  const packagingFee = restaurant?.packagingFee ?? 0;
  const taxRate = (settings?.taxRatePercent ?? 5) / 100;
  const estimatedTax = Math.round(subtotal * taxRate * 100) / 100;
  const estimatedTotal = useMemo(
    () => Math.max(0, subtotal + deliveryFee + packagingFee + estimatedTax + tip - (couponDiscount ?? 0)),
    [subtotal, deliveryFee, packagingFee, estimatedTax, tip, couponDiscount],
  );
  ...
  function composedInstructions() {
    const parts = [...presetInstructions];
    if (noCutlery) parts.push("No plastic cutlery, please");
  ...
  async function placeOrder() {
    ...
    if (restaurant && subtotal < restaurant.minOrderAmount) {
      setFormError(`Minimum order amount for this restaurant is ₹${restaurant.minOrderAmount}.`);
      return;
    }
    setFormError(null);
    setPlacing(true);
    try {
      const res = await api.post<{ order: Order; payment: Record<string, unknown> | null }>("/orders", {
        restaurantId: cart.restaurantId,
      ...
      const p = payment as { mock?: boolean; keyId?: string; razorpayOrderId?: string; amount?: number; currency?: string };

      if (p.mock) {
        await api.post(`/payments/mock/${order.id}`);
        clearCart();
        show("Payment successful (demo mode). Order placed!", "success");
        router.push(`/orders/${order.id}`);
        return;
      }
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) => {
          try {
            ...
            show(e instanceof ApiError ? e.message : "Payment verification failed.", "error");
            router.push(`/orders/${order.id}`);
          }
        },
        theme: { color: "#FF6A00" },
    ...
    <div className="container-glido py-8 max-w-2xl">
      ...
      <section className="card-glido p-4 mb-4">
        ...
        {addresses && addresses.length > 0 && !showAddAddress && (
          <div className="space-y-2">
            {addresses.map((a) => (
              <label key={a.id} className="flex items-start gap-2 p-3 border border-[var(--glido-border)] rounded-lg cursor-pointer has-[:checked]:border-[var(--glido-primary)] has-[:checked]:bg-[var(--glido-primary-light)]">
                <input
                  type="radio"
                  name="address"
                  className="mt-1"
      ...
      <section className="card-glido p-4 mb-4">
        ...
        <div className="flex gap-2">
          <input
            className="input-glido"
            placeholder="Enter coupon code (try GLIDO50)"
            value={couponCode}
            onChange={(e) => {
      ...
      <section className="card-glido p-4 mb-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="font-semibold">Opt out of plastic cutlery</h2>
            <p className="text-xs text-[var(--glido-muted)] mt-0.5">Thank you for caring about the planet</p>
          </div>
          <button
            type="button"
            role="switch"
            ...
            style={{ background: noCutlery ? "var(--glido-primary)" : "var(--glido-border)" }}
          >
            <span
              className="absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-all"
              style={{ left: noCutlery ? "1.375rem" : "0.125rem" }}
            />
          </button>
        </div>
      </section>
          ...
          {PRESET_INSTRUCTIONS.map((label) => {
            ...
            return (
              <button
                ...
                style={
                  active
                    ? { background: "var(--glido-primary-light)", borderColor: "var(--glido-primary)", color: "var(--glido-primary-dark)" }
      ...
      <section className="card-glido p-4 mb-4">
        <h2 className="font-semibold mb-3">Tip your delivery partner</h2>
        <p className="text-xs text-[var(--glido-muted)] mb-3">100% goes to your rider. Totally optional.</p>
        <div className="flex flex-wrap gap-2">
          {[20, 30, 50].map((amount) => (
            <button
              ...
              style={
                tip === amount
                  ? { background: "var(--glido-primary)", borderColor: "var(--glido-primary)", color: "white" }
      ...
      <section className="card-glido p-4 mb-4 text-sm">
        ...
        <div className="space-y-3 mb-3 pb-3 border-b border-[var(--glido-border)]">
          {cart?.items.map((item) => (
            <div key={item.menuItemId + item.addonNames.join(",")} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.addonNames.length > 0 && (
                  <p className="text-xs text-[var(--glido-muted)] truncate">+ {item.addonNames.join(", ")}</p>
                )}
              </div>
              <p className="font-medium shrink-0">₹{((item.price + item.addonsPrice) * item.quantity).toFixed(2)}</p>
            </div>
          ))}
        </div>
        <div className="space-y-1.5">
          ...
          <div className="flex justify-between"><span className="text-[var(--glido-muted)]">Delivery fee</span><span>₹{deliveryFee.toFixed(2)}</span></div>
          <div className="flex justify-between"><span className="text-[var(--glido-muted)]">Packaging fee</span><span>₹{packagingFee.toFixed(2)}</span></div>
