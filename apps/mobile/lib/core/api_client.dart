...
/// Same origin the web app talks to — override with --dart-define=API_BASE_URL=...
/// when running against a device/emulator that can't reach localhost directly
/// (e.g. Android emulator needs http://10.0.2.2:4000/api).
...
/// Resolves a stored image value to a renderable URL — mirrors
/// apps/web/src/lib/api.ts#resolveMediaUrl.
...
/// Thin wrapper around Dio that mirrors the web app's lib/api.ts:
/// persists JWTs, retries once on 401 after a token refresh.
class ApiClient {
  ApiClient._internal() {
  ...
  Future<void> loadTokens() async {
    ...
    _accessToken = prefs.getString('glido_access_token');
    _refreshToken = prefs.getString('glido_refresh_token');
  }
  ...
  Future<void> setTokens(String access, String refresh) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('glido_access_token', access);
    await prefs.setString('glido_refresh_token', refresh);
  }
  ...
  Future<void> clearTokens() async {
    ...
    _refreshToken = null;
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove('glido_access_token');
    await prefs.remove('glido_refresh_token');
  }
